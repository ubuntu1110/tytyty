// frontend/src/components/AccountantExpenses/AccountantExpensesAnalytics.tsx
...
interface AccountantExpensesAnalyticsProps {
    analytics: ExpenseAnalytics;
}

const AccountantExpensesAnalytics: React.FC<AccountantExpensesAnalyticsProps> = ({ analytics }) => {
    const categoryDataRub = analytics.category_data_rub || {};
    const categoryDataUsd = analytics.category_data_usd || {};

    return (
        <>
            <h2 className="text-xl text-yellow-400 mt-4">Аналитика</h2>
            ...
            {/* Рубли */}
            <h3 className="text-lg text-yellow-300 mt-4">По рублям (₽):</h3>
            <p className="text-white">Всего потрачено: {analytics.total_spent_rub.toFixed(2)}₽, Всего единиц: {analytics.total_count_rub.toFixed(2)}</p>
                    ...
                    Object.keys(categoryDataRub).map(cat => (
                        <tr key={cat} className="hover:bg-gray-800">
                            ...
                            <td className="border px-4 py-2 border-yellow-500 text-white">
                                {categoryDataRub[cat].spent.toFixed(2)}
                            </td>
                            <td className="border px-4 py-2 border-yellow-500 text-white">
                                {categoryDataRub[cat].count.toFixed(2)}
                            </td>
                            <td className="border px-4 py-2 border-yellow-500 text-white">
                                {categoryDataRub[cat].avg_per_item.toFixed(2)}
                            </td>
                        </tr>
                    ))
                )}
            ...
            {/* Доллары */}
            <h3 className="text-lg text-yellow-300 mt-4">По долларам ($):</h3>
            <p className="text-white">Всего потрачено: {analytics.total_spent_usd.toFixed(2)}$, Всего единиц: {analytics.total_count_usd.toFixed(2)}</p>
            <table className="min-w-full border-collapse border border-yellow-500 mt-2">
                <thead>
                <tr className="bg-yellow-700 text-white">
                    <th className="border px-4 py-2 border-yellow-500">Категория</th>
                ...
                {Object.keys(categoryDataUsd).length === 0 ? (
                    ...
                    Object.keys(categoryDataUsd).map(cat => (
                        <tr key={cat} className="hover:bg-gray-800">
                            <td className="border px-4 py-2 border-yellow-500 text-white">{cat}</td>
                            <td className="border px-4 py-2 border-yellow-500 text-white">
                                {categoryDataUsd[cat].spent.toFixed(2)}
                            </td>
                            <td className="border px-4 py-2 border-yellow-500 text-white">
                                {categoryDataUsd[cat].count.toFixed(2)}
                            </td>
                            <td className="border px-4 py-2 border-yellow-500 text-white">
                                {categoryDataUsd[cat].avg_per_item.toFixed(2)}
                            </td>
                        </tr>
                    ))
                )}
...
export default AccountantExpensesAnalytics;
