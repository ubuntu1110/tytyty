// frontend/src/components/AdminExpenses/AdminExpensesList.tsx
...
const AdminExpensesList: React.FC<AdminExpensesListProps> = ({ expenses, onEdit }) => {
    return (
                ...
                expenses.map(exp => (
                    <tr key={exp.id} className="hover:bg-gray-800">
                        ...
                        <td className="border px-4 py-2 border-yellow-500 text-white">
                            {exp.quantity !== undefined && exp.quantity !== null ? exp.quantity.toFixed(2) : '-'}
                        </td>
